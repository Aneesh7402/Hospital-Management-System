<html>
    ...
    <body style="background-image: url(Images/Pic63.jpg);">
        <div class="header">
            <ul>
                <li style="float: left; border-right: none;"><a href="Login.php" class="logo"><img src="Images/Pic2.png" width="70px" height="60px"> <strong> Apollo </strong> Online Apppointment System </a></li>
		...
		while($rs=$results->fetch_assoc()) {
			...
			$results2=$conn->query($sql2);
				while($rs2=$results2->fetch_assoc()) {
					$sql3="select * from Branch where CID=".$rs["CID"];
					$results3=$conn->query($sql3);
		while($rs3=$results3->fetch_assoc()) {
			
		?>
		<option value="<?php echo $rs["Timestamp"]; ?>"><?php echo "Patient: ".$rs["Fname"]." Date: ".$rs["DOV"]." -Dr.".$rs2["name"]." -Branch: ".$rs3["name"]." - Booked on:".$rs["Timestamp"]; ?></option>
		<?php
		}
		}
		}
