...
<html>
    ...
    function getDay(val) 
    {
	    var CID=document.getElementById("Branch-list").value;
    ...
    <body style="background-image: url(Images/Pic123.jpg);">
	<div class="header">
    <ul>
	...
	<div class="sucontainer" style="background-image:url(Images/Pic12.jpg)">
		...
		<select name="city" id="city-list" class="demoInputBox"  onChange="getBranch(this.value);" style="width:100%;height:35px;border-radius:9px">
		...
		<label style="font-size:20px ; font-family:cursive;color:black">Branch:</label><br>
		<select id="Branch-list" name="cid" onChange="getDoctorday(this.value);" style="width:100%;height:35px;border-radius:9px">
        <option value="" style="font-family:cursive;color:black">Select Branch</option>
        <?php
...
if(isset($_POST['submit']))
{
		...
		if(!empty($_POST['fname'])&&!empty($_POST['gender'])&&!empty($_POST['cid'])&&!empty($_POST['doctor']) && !empty($_POST['DOV']))
		{
			...
			while($rs=$results->fetch_assoc())
			{
				if(strtolower(strtolower($rs["day"]))==strtolower($compareday))
				{
					$flag++;
					break;
				}
